...
def circle_it_rev(ruler, roadrunner_wile, lili, lilibeth, enc, omega):
    l, r = [[[0 for x in range(16)] for y in range(32)] for _ in '01'] 
    for i in range(16):
        l[ruler - 1][i] = enc[i + 16]
        r[ruler - 1][i] = enc[i]
    for i in range(ruler - 1, 0, -1):
        for y in range(16):
            r[i - 1][y] = l[i][y]
        roadrunner_wile = roadrunner(r[i-1], roadrunner_wile, lili, lilibeth, omega)
        for y in range(16):
            l[i-1][y] = (r[i][y] ^ roadrunner_wile[y]) % 256
    msg = b""
    for i in range(16):
        msg += bytes([l[0][i]])
    for i in range(16):
...
def checkers(key):
    ...
    l, r = [[[0 for x in range(16)] for y in range(32)] for _ in '01'] 
    roadrunner_wile, lili, lilibeth, enc, l, r, omega = persian_init(key, roadrunner_wile, lili, lilibeth, enc, l, r, omega, Omega)
    return roadrunner_wile, lili, lilibeth, omega
...
flagencAA = ""
flagenc = "1db84f7bcf3bc5a35b307d3d47b08fbe87046290b18cc5f4f2cf08ac976ac821fee509010e4b583bf37555729207eea84ee3a28c106eb8e9ae8328fe993f143455273ed694d99bf80809948c5fed958deb0ddc2fb37943c5643e61b884b58e7c"
flagencAA = bytes.fromhex(flagencAA)
flagenc = bytes.fromhex(flagenc)

key2 = xors(b"A" * 32, flagencAA[:32]) # this was used to encrypt flag[:32] on 
key2 = [key2[i] for i in range(20)]

roadrunner_wile, lili, lilibeth, omega = checkers(key2)
flag1 = circle_it_rev(6, roadrunner_wile, lili, lilibeth, flagencAA[32:64], omega)

key3 = xors(flag1, flagencAA[32:64])
key3 = [key3[i] for i in range(20)]

roadrunner_wile, lili, lilibeth, omega = checkers(key3)
flag2 = circle_it_rev(6, roadrunner_wile, lili, lilibeth, flagencAA[64:96], omega)

key4 = xors(flag2, flagencAA[64:96])
key4 = [key4[i] for i in range(20)]

roadrunner_wile, lili, lilibeth, omega = checkers(key4)
flag3 = circle_it_rev(6, roadrunner_wile, lili, lilibeth, flagencAA[96:128], omega)
